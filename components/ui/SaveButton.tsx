...
const SaveButton = ({ loading = false, className, ...props }: SaveButtonProps) => {
  return (
    <button className="relative cursor-pointer m-2 py-1 px-4 text-center font-barlow inline-flex justify-center text-base uppercase text-white rounded-lg border-solid transition-transform duration-300 ease-in-out group outline-offset-4 focus:outline focus:outline-white focus:outline-offset-4 overflow-hidden"
      title="Salvar Alterações"
      disabled={loading}
      {...props}
    >
      <div className="flex items-center justify-center gap-2">
        <span className="relative z-20">{loading ? "Salvando..." : "Salvar"}</span>
        {loading && (
