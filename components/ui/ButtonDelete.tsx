...
const ButtonDelete = ({ className, ...props }: ButtonProps) => {
  return (
    <Button
      {...props}
      className={cn(
        "group relative flex h-14 w-14 flex-col items-center justify-center overflow-hidden rounded-xl border-2 hover:bg-red-800 border-background",
        className
      )}
      aria-label={props?.title ?? "Delete"}>
      ...
      <svg width={16} fill="none" viewBox="0 0 39 7" className="origin-right duration-500 group-hover:rotate-90 mb-[-0.8rem] ">
